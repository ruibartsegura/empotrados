...
void* thread_core(void *arg) {
    ...
    long long latency = 0;
    long long *latencies = malloc(sizeof(long long) * 10000); // some large number or estimate
    if (!latencies) {
        perror("malloc");
        pthread_exit(NULL);
    }
    ...
    while (time_flag) {
        ...
        latencies[num_iter] = latency;
        num_iter ++;

    }
    // Añadir al array latencies
    args->latencies[args->id] = latencies;
    free(latencies);
    return 0;
}
...
int main (int argc, char* argv[]) {
    ...
    long long latencies[N]; // Array to store the average latencie of each core
    //long long max_latencies[N]; // Array to store the maximun latencie of each core
